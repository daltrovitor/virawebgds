...
function extractTextItems(lines: string[], entity: EntityType): ExtractedItem[] {
    ...
    const validLines = lines.filter(l => l.length >= 3 && !isHeaderLike(l))
...
async function processWithOCR(buffer: Buffer, entity: EntityType): Promise<ExtractedItem[]> {
    ...
    if (!fs.existsSync(workerPath)) throw new Error("Motor OCR não encontrado. Reinstale as dependências.")
    try {
        const worker = await Tesseract.createWorker("por", 1, { workerPath })
        const { data: { text } } = await worker.recognize(buffer)
        await worker.terminate()
        if (!text || text.length < 10) { console.warn("OCR retornou texto insuficiente."); return [] }
        const items = extractEntitiesFromText(text, entity)
        return mergeVerticalBlocks(items, entity)
    } catch (error: any) {
        console.error("Erro no worker OCR:", error)
        throw new Error("Erro ao processar imagem (OCR). Tente PDF.")
    }
}
